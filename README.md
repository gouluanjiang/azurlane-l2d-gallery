...
云端按排期检查并发布数据后，本机仍需运行“一键更新”才能接收；无需保持图鉴窗口打开。搜索网页、检查新资料和下载新图片需要联网，已有本地原图与图解可以离线查看。
...
| 云端验收 | 已完成真正增量实测：移除 9 月 8 日四款后，云端从 98 款重新收录至 102 款并校验四张原图；本机实际新增 4、下载 4。6 款返场未重复收录、待核对 0；再次同步为 `unchanged`。 |

截至 2026-09-12，本机已同步云端版本 `20260912T092314483Z-b101b7589b33`。旧 27 款补图继续暂停；新增 7 款暂用玩法搜索入口，不把立绘、视频封面或普通文字攻略当作玩法图解。
...
此前新增 7 款的本机实测保留在提交 [77aa4b6](https://github.com/gouluanjiang/azurlane-l2d-gallery/commit/77aa4b6aaf0aeb02cd193397cceee23bc40aaa10)。之后又移除 9 月 8 日四款及活动目录中的原图，完成了 [98→102 的真实云端增量实测](./云端增量实测-2026-09-08.md)：Actions 自动发布 [be8aad5](https://github.com/gouluanjiang/azurlane-l2d-gallery/commit/be8aad51318970f36111cbfd799c91f0ba52833c)，本机新增并重新下载各 4，全部记录与测试前一致。
...
本机已清理优化前网页、早期试作目录、临时云端测试工程、未引用图片及失效的旧原图下载脚本，只保留一套可运行工程。`scripts`、`tests`、`.github` 与精确图解下载映射属于完成版维护文件；`.l2d-update` 中的回退资料及 `verification` 中的验收记录仍保留。
